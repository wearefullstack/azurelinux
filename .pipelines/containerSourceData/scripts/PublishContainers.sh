...
do
    ...
    do
        ...
        if [[ $container_registry != "$TARGET_ACR" ]]; then
            ...
            echo "Proto2: az acr repository delete --name $container_registry --image $amd64_retagged_image_name --yes"
            ...
            if [[ $ARCHITECTURE_TO_BUILD == *"ARM64"*  ]]; then
                ...
                echo "Proto2: az acr repository delete --name $container_registry --image $arm64_image --yes"
            fi
